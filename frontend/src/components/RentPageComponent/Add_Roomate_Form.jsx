...
const AddRequirement = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        formData.append("approx_rent", approxRent);
        formData.append("room_type", roomType);
        formData.append("highlights", Array.from(highlights).join(", "));
        formData.append("post", post);
        formData.append("listing_type", "roommates");
        formData.append("occupancy", occupancy);
        formData.append("number_of_people", numberOfPeople);
        ...
        try {
            ...
            setRequirements((prevRequirements) => [
                ...prevRequirements,
                {
                    ...
                    approx_rent: approxRent,
                    room_type: roomType,
                    highlights: Array.from(highlights).join(", "),
                    post,
                    listing_type: "roommates",
                    occupancy,
                    number_of_people: numberOfPeople,
    ...
    const handleHighlightToggle = (highlight) => {
        setHighlights((prevHighlights) => {
            const newHighlights = new Set(prevHighlights);
            if (newHighlights.has(highlight)) {
                newHighlights.delete(highlight);
            } else {
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="mt-12">
                    ...
                    <div className="mt-2 grid grid-cols-2 md:grid-cols-3 gap-4">
                        {highlightProperty.map((highlight) => (
                            <button
                                key={highlight}
                                type="button"
                                className={`px-4 py-2 border rounded-md text-sm font-medium ${
                                    highlights.has(highlight)
                                        ? "color"
                                        : "hover:bg-gray-100"
                                }`}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-12">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700">Exists Member Count</label>
                        <input
                            type="text"
                            value={occupancy}
                            onChange={(e) => setOccupancy(e.target.value)}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-12">
                    <div>
                        <label className="block text-sm font-medium text-gray-700">Occupation Member Count</label>
                        <input
                            type="text"
                            value={numberOfPeople}
                            onChange={(e) => setNumberOfPeople(e.target.value)}
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700">Upload House Image</label>
                        <input
                            type="file"
                            accept="image/*"
                            ref={fileInputRef}
                ...
                <div className="flex justify-end mt-12">
                    <button
                        type="button"
                        onClick={handleCancel}
                        className="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 mr-4 hover:bg-gray-100"
                    >
                        Cancel
                    </button>
                    <button
                        type="submit"
                        className="px-4 py-2 color text-white rounded-md text-sm font-medium hover:bg-blue-600"
                    >
                        Submit
                    </button>
                </div>
